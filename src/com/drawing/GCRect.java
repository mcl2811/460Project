...
public class GCRect implements GShape, Comparable<GCRect> {
	...
	GCRect(final GL2 gl, float vertex2f[], float speed) {
	...
	public void reloadThings(final GL2 gl, boolean resetPosFlag) {
		...
		final int MIN_HEIGHT = (int) GLUTCanvas.DRAWING_HEIGHT/16 + 30 , 
		...
		//vertex2f[3] = this.vertex2f[3];
		vertex2f[4] = this.vertex2f[4];
		
	}
		// calculating texture height
		texture_height = (int) ((this.getWidth() / this.getHeight() * TEXHEIGHTCORRECTION + 40));

		// speed of movement
		final float MIN_SPEED = 0.25f, MAX_SPEED = 1.0f;
		//speed = MIN_SPEED + (float) rand.nextDouble() * MAX_SPEED;
		// load a random scene texture
		final int MAX_SCENE = 4;
		int texture_number = 1 + rand.nextInt(MAX_SCENE);
		String textureName = "/world/forest/scenes/Scene" + texture_number + ".png";
	...
	public int compareTo(GCRect paramRect) {
