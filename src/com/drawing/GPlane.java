...
public class GPlane implements GShape {
	...
	final static int FLY = 0, WALK = 1, RUN = 2, JUMP = 3;
	final int TOTAL_ANIM = 1;

	// name of the animations
	final String fileName[] = { "fly"};
	// how many frames do we have for each animation
	final int frameNo[] = { 50};
	...
	int currentAnimation = FLY;

	// constructor to initialize the sprite
	GPlane(final GL2 gl, int x, int y, long interval, float w, float h) {
		...
		for (int i = 0; i < TOTAL_ANIM; i++) {
			...
			for (int j = 0; j < 14; j++) {
				...
				String name = "/world/Plane/png/Plane/" + fileName[i] + (j + 1) + ".png";
	...
	private void renderQuad(final GL2 gl, Texture texture) {
		...
		gl.glColor3f(1f, 1f, 1f); // drawing color
